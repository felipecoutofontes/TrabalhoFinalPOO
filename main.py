    ...
    while run:
        ...
        if not game_over:
            if world.health is None:
                world.health = 100  # Ou qualquer valor que você considerar apropriado
            if world.health <= 0:
                # Código para tratar quando a saúde for menor ou igual a 0
                ...
                if world.level > c.TOTAL_LEVELS:
                        ...
                        # Garantir que `world.health` e `world.money` não sejam None
                        world.health = world.health if world.health is not None else 100  # Valor padrão para saúde
                        world.money = world.money if world.money is not None else 250  # Valor padrão para moedas
